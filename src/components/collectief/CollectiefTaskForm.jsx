        ...
        {!usesArrivalDeadline && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white border border-slate-200 rounded-lg p-3">
            <div className="space-y-2">
              ...
              <Input type="number" min="1" value={form.repeat_count || 1} onChange={(e) => handleChange("repeat_count", Number(e.target.value) || 1)} />
            </div>
            <div className="space-y-2">
              <Label className="text-xs font-semibold uppercase tracking-wider text-slate-500">Min. tijd ertussen (min)</Label>
              <Input type="number" min="0" value={form.min_minutes_between_visits || 0} onChange={(e) => handleChange("min_minutes_between_visits", Number(e.target.value) || 0)} disabled={Number(form.repeat_count || 1) <= 1} />
            </div>
            <p className="md:col-span-2 text-xs text-slate-500">Voor meerdere rondes binnen hetzelfde tijdvenster maakt de planning ze verspreid aan.</p>
          </div>
        )}
