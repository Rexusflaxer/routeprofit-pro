...
export default function NavigationStatusPanel({ route, stops, visitedIds, tracking, onStart, onFinish }) {
  ...
  return (
    <div className={`absolute inset-x-3 bottom-3 z-[500] rounded-xl border border-white/10 bg-slate-950/90 text-white shadow-2xl backdrop-blur md:left-auto md:right-4 md:w-80 ${tracking ? "p-3" : "p-4"}`}>
      {tracking && (
      ...
      <div className="flex cursor-pointer items-start justify-between gap-3 pr-6" onClick={() => tracking && setExpanded(value => !value)}>
        <div>
      ...
      <div className={`${tracking ? "mt-3 max-h-20" : "mt-4 max-h-44"} space-y-2 overflow-y-auto pr-1`}>
          ...
          return (
            <div key={stop.id} className="flex items-start gap-2 rounded-lg bg-white/5 p-2">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold leading-tight">{showCompact ? "Volgende taak: " : ""}{stop.sequence}. {stop.name}</p>
