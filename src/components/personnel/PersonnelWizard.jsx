...
function getCreationTitle(form) {
  return deriveRelationshipType(form) === "self_employed" ? "ZZP'er aanmaken" : "Loondienstmedewerker aanmaken";
}
...
export default function PersonnelWizard({ initialValues = {}, onClose, onSaved }) {
  ...
  const creationTitle = getCreationTitle(form);
    ...
    <Card className="border-0 shadow-lg">
      <CardHeader className="border-b border-border pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">{creationTitle}</CardTitle>
      ...
      <div className="flex items-center justify-between border-t border-border px-6 py-4">
        ...
        <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}>
          <Save className="mr-1 h-4 w-4" />{saveMutation.isPending ? "Aanmaken..." : creationTitle}
        </Button>
      </div>
    </Card>
  );
