...
export default function GlobalSearchOverlay({ open, onClose }) {
  ...
  return <AnimatePresence>{open && <motion.div className="fixed inset-0 z-[100] overflow-hidden bg-background/35 px-4 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    <button type="button" onClick={onClose} aria-label="Zoeken sluiten" className="absolute right-5 top-5 rounded-full border border-border/70 bg-card/40 p-2 text-muted-foreground shadow-lg backdrop-blur-xl hover:text-foreground"><X className="h-5 w-5" /></button>
