...
export default function GlobalSearchOverlay({ open, onClose }) {
  ...
  return <AnimatePresence>{open && <motion.div className="fixed inset-0 z-[100] overflow-hidden bg-background/70 px-4 backdrop-blur-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    <button type="button" onClick={onClose} aria-label="Zoeken sluiten" className="absolute right-5 top-5 rounded-full border border-border bg-card/70 p-2 text-muted-foreground hover:text-foreground"><X className="h-5 w-5" /></button>
    <motion.div className="mx-auto w-full max-w-3xl" initial={{ marginTop: "36vh", scale: .98 }} animate={{ marginTop: query ? "3.5rem" : "36vh", scale: 1 }} transition={{ type: "spring", stiffness: 260, damping: 28 }}>
      <div className="flex h-14 items-center gap-3 rounded-2xl border border-border bg-card/80 px-4 shadow-2xl backdrop-blur-2xl"><Search className="h-5 w-5 shrink-0 text-muted-foreground" /><input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} placeholder="Waar wilt u naar zoeken?" aria-label="Globaal zoeken" className="h-full min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-muted-foreground" />{query && <button type="button" onClick={() => setQuery("")} aria-label="Zoekterm wissen" className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>}</div>
