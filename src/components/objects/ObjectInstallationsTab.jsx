...
export default function ObjectInstallationsTab({ object, view, selectedRow, searchTerm, onSearchChange, onOpenCreate, onOpenEdit, onCloseView }) {
  ...
  const rows = useMemo(() => {
    ...
    return installations.filter(item => !term || [item.name, installationTypeLabel(item), item.brand, item.model, item.serial_number, item.external_reference, item.control_panel_location, item.monitoring_provider_name, item.installer_name].some(value => String(value || "").toLowerCase().includes(term)));
  ...
  useEffect(() => {
    if (view === "edit" && !query.isLoading && !query.isError && !currentInstallation) onCloseView();
  }, [currentInstallation, onCloseView, query.isError, query.isLoading, view]);
