...
export default function WarningAvailabilityHoverTooltip({ hover }) {
  ...
  return createPortal(
    <div className="pointer-events-none fixed z-[100] min-w-32 rounded-md border border-border bg-popover px-2.5 py-2 text-xs text-popover-foreground shadow-lg" style={{ left: hover.x, top: hover.y }}>
      ...
      {hover.interval?.alternative ? <div className="mt-1.5 flex items-center gap-1.5 font-medium text-primary"><Shuffle className="h-3.5 w-3.5" /><span>Aangepast alternatief</span></div> : hover.interval?.repeating && <div className="mt-1.5 space-y-0.5 text-primary"><div className="flex items-center gap-1.5 font-medium"><Repeat2 className="h-3.5 w-3.5" /><span>Wekelijkse herhaling</span></div><div className="pl-5 text-[11px] text-muted-foreground">{hover.interval.repeatUntil ? `Eindigt op ${formatDate(hover.interval.repeatUntil)}` : "Herhaalt voor onbepaalde tijd"}</div></div>}
