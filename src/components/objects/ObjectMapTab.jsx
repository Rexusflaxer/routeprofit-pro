...
export default function ObjectMapTab({ object, onRegisterNavigationGuard }) {
  ...
  const canSeedExactSelection = candidatesQuery.isSuccess
    && !candidatesQuery.isFetching
    && automaticBagFeatureIds.length > 0;
  ...
  const manualArea = featureCollectionAreaSquareMeters(form?.manual_building_geojson);
  ...
  const finishDrawing = () => {
    ...
    updateWithHistory(current => target === "terrain"
      ? { ...current, object_area_geojson: appendPolygon(current.object_area_geojson, drawingPoints, { source: "user_drawn", source_feature_id: `terrain:${featureId}` }) }
      : { ...current, building_selection_mode: "manual", manual_building_geojson: appendPolygon(current.manual_building_geojson, drawingPoints, { source: "manual", source_feature_id: `manual:${featureId}` }) });
    setDrawingTarget(null);
    setDrawingPoints([]);
    setDrawingSaveNotice(false);
  };
  ...
  const moveVertex = (target, reference, coordinate) => {
    ...
    setOverlapFingerprint(null);
    setServerOverlapConflicts([]);
    setForm(current => target === "terrain"
      ? { ...current, object_area_geojson: replaceVertex(current.object_area_geojson, reference, coordinate) }
      : { ...current, manual_building_geojson: replaceVertex(current.manual_building_geojson, reference, coordinate) });
  };
    ...
    <div className="flex min-h-[700px] flex-col bg-card/30 backdrop-blur-xl">
      <div className="flex flex-col gap-3 border-b border-border/70 bg-card/30 px-4 py-3 backdrop-blur-xl xl:flex-row xl:items-center xl:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Koppel de juiste BAG-gebouwen en leg de operationele terreinbegrenzing vast.</p>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border/70 bg-card/35 p-2 backdrop-blur-xl">
            {workspace === "buildings" ? <>
              ...
              <Button type="button" size="sm" variant={editingTarget === "terrain" ? "secondary" : "ghost"} onClick={() => { cancelDrawing(); setEditingTarget(value => value === "terrain" ? null : "terrain"); }} disabled={readOnly || drawingPoints.length > 0 || !form.object_area_geojson.features.length}><MousePointer2 className="h-4 w-4" /> Hoekpunten bewerken</Button>
            </>}
        ...
        <aside className="space-y-3">
          ...
          {workspace === "buildings" ? <>
            <section className="space-y-2 rounded-xl border border-border/70 bg-card/45 p-4 backdrop-blur-xl">
              ...
              <ChoiceCard active={form.building_selection_mode === "automatic"} icon={RotateCcw} title="Automatisch bepalen" description="Gebruik de bestaande adresnabijheid zolang geen exacte selectie nodig is." disabled={readOnly} onClick={() => updateWithHistory(current => ({ ...current, building_selection_mode: "automatic", selected_bag_feature_ids: [], manual_building_geojson: emptyFeatureCollection() }))} />
              <ChoiceCard active={form.building_selection_mode === "manual"} icon={Building2} title="Exact vastleggen" description="Gebruik uitsluitend de geselecteerde BAG-panden en handmatig getekende contouren." disabled={readOnly || (form.building_selection_mode === "automatic" && !canSeedExactSelection)} onClick={() => updateWithHistory(current => ({
                ...current,
            ...
            <section className="rounded-xl border border-border/70 bg-card/45 backdrop-blur-xl">
              ...
              <div className="max-h-72 divide-y divide-border/60 overflow-y-auto">
                ...
                {form.building_selection_mode === "manual" && form.manual_building_geojson.features.map((_, index) => <div key={`manual-${index}`} className="flex items-center gap-3 p-3"><span className="h-2.5 w-2.5 rounded-full bg-violet-500" /><div className="min-w-0 flex-1"><p className="text-xs font-medium">Handmatige gebouwcontour {index + 1}</p><p className="mt-0.5 text-[11px] text-muted-foreground">Zelf ingetekend</p></div><Button type="button" variant="ghost" size="icon" className="h-8 w-8" disabled={readOnly} onClick={() => updateWithHistory(current => ({ ...current, manual_building_geojson: removeFeature(current.manual_building_geojson, index) }))} aria-label={`Handmatige gebouwcontour ${index + 1} verwijderen`}><Trash2 className="h-3.5 w-3.5" /></Button></div>)}
              </div>
              ...
              {buildingArea > 0 && <div className="border-t border-border/70 px-4 py-3 text-xs text-muted-foreground">Totale gebouwoppervlakte: {formatArea(buildingArea)}{manualArea > 0 ? ` · handmatig ${formatArea(manualArea)}` : ""}</div>}
            </section>
          </> : <>
            <section className="rounded-xl border border-border/70 bg-card/45 p-4 backdrop-blur-xl">
