...
import {
  deleteWarningAvailabilityOverride,
  deleteWarningAvailabilityOverrideKey,
  upsertWarningAvailabilityOverrides,
  upsertWarningAvailabilityOverridesKey,
...
export default function WarningAvailabilityTimelineDialog({ record, open, onOpenChange, onOverridesChanged, readOnly = false }) {
  ...
  const saveDrafts = useMutation({
    mutationFn: reason => {
      if (!saveKeyRef.current) saveKeyRef.current = upsertWarningAvailabilityOverridesKey();
      return upsertWarningAvailabilityOverrides({ record, drafts: draftsRef.current, reason, slotsToPeriods: slotsToOverridePeriods, idempotencyKey: saveKeyRef.current });
    },
    onSuccess: async () => { await refresh(); saveKeyRef.current = null; draftsRef.current = {}; setDrafts({}); setEditing(false); setSaveReasonOpen(false); },
    onError: async error => { if (error.status === 409) { saveKeyRef.current = null; await refresh(); } },
  });
  const removeOverride = useMutation({
    mutationFn: () => {
      if (!deleteKeyRef.current) deleteKeyRef.current = deleteWarningAvailabilityOverrideKey();
      return deleteWarningAvailabilityOverride({ record, override: selection.override, date: localDateKey(selection.date), idempotencyKey: deleteKeyRef.current });
    },
    onSuccess: async () => { await refresh(); deleteKeyRef.current = null; setSelection(null); },
    onError: async error => { if (error.status === 409) { deleteKeyRef.current = null; await refresh(); } },
  });

  useEffect(() => {
    if (!open) return;
    setWeekCount(12); setCanScrollUp(false); setEditing(false); setSelection(null); setSaveReasonOpen(false); saveKeyRef.current = null; deleteKeyRef.current = null; draftsRef.current = {}; setDrafts({});
  ...
  const cancelEditing = () => { saveKeyRef.current = null; draftsRef.current = {}; setDrafts({}); setEditing(false); };
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[90vh] max-w-[calc(100vw-2rem)] overflow-hidden p-0 sm:max-w-6xl">
      <DialogHeader className="flex-row items-center justify-between space-y-0 px-6 py-4 pr-14">
        ...
        {!readOnly && <Button type="button" variant={editing ? "secondary" : "ghost"} size="sm" onClick={() => editing ? cancelEditing() : setEditing(true)}><Pencil className="h-4 w-4" /> {editing ? "Bewerken actief" : "Wijzigen"}</Button>}
      </DialogHeader>
        ...
        <div className="min-w-[900px]">
          ...
          <div ref={scrollRef} className="h-[336px] touch-pan-y snap-y snap-mandatory overflow-y-auto overscroll-contain" onScroll={handleScroll}>
            {dates.map(date => {
              ...
              const override = overrideForDate(record, date);
              const intervals = intervalsFor(date);
              return <WarningTimelineRow key={date.toISOString()} date={date} dayIndex={dayIndex} now={now} available={intervals.available} emergency={intervals.emergency} override={override || Boolean(drafts[localDateKey(date)])} editing={editing && !readOnly} painting={painting} onPaint={(slot, start) => paint(date, slot, start)} onHover={event => showHover(event, date, intervals, override)} onHoverEnd={() => setHover(null)} onOpenOverride={event => { if (!readOnly) setSelection({ date, override, x: event.clientX, y: event.clientY }); }} />;
            })}
          </div>
