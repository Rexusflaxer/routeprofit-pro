...
export default function ObjectKeyBrandStep({ keyType, value, knownBrands, onChange }) {
  ...
  return <><StepHeading icon={Factory} title="Van welk merk is het toegangsmiddel?" description="De lijst bevat gangbare merken voor het gekozen type. Een ander merk kan direct worden toegevoegd." />{custom ? <div className="max-w-xl space-y-3 rounded-xl border border-primary/30 bg-card/45 p-4 shadow-sm backdrop-blur-xl"><Field label="Merk" htmlFor="custom-key-brand" required><Input id="custom-key-brand" value={value} onChange={event => onChange(event.target.value)} placeholder="Vul het merk in" autoFocus /></Field><Button type="button" size="sm" variant="outline" onClick={() => { setCustom(false); onChange(""); }}>Terug naar merken</Button></div> : <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{brands.map(brand => <ChoiceCard key={brand} selected={value === brand} onClick={() => onChange(brand)} title={brand} />)}<ChoiceCard selected={false} onClick={() => { setCustom(true); onChange(""); }} title="Ander merk" description="Vul het merk handmatig in." icon={Plus} /></div>}</>;
}
