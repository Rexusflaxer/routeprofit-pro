...
export default function ObjectWarningAddressesDesktop({ rows, onEdit, onDelete, editingId, deletingId, onDragEnd, reorderDisabled, actionsDisabled, expandedId, onToggleExpanded }) {
  return (
        ...
        <Droppable droppableId="warning-addresses-desktop" direction="vertical">
          {provided => <TableBody ref={provided.innerRef} {...provided.droppableProps}>
            {rows.map((row, index) => <Draggable key={row.id} draggableId={`warning:${row.id}`} index={index} isDragDisabled={reorderDisabled}>
              {(drag, snapshot) => <TableRow ref={drag.innerRef} {...drag.draggableProps} onClick={() => onToggleExpanded(row.id)} aria-expanded={expandedId === row.id} className={`cursor-pointer ${editingId === row.id ? "bg-primary/5" : "hover:bg-muted/25"} ${snapshot.isDragging ? "bg-card shadow-lg" : ""}`}>
                ...
                <TableCell className="min-w-64 text-sm"><div className="flex items-center justify-between gap-2"><span>{warningAvailabilityLabel(row)}</span><ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${expandedId === row.id ? "rotate-180" : ""}`} /></div>{expandedId === row.id && <WarningAvailabilityDetails record={row} />}</TableCell>
