...
export default function WarningOverrideList({ overrides, onDelete, deleting }) {
  if (!overrides.length) return <p className="rounded-md border border-dashed border-border bg-muted/10 p-4 text-center text-xs text-muted-foreground">Nog geen specifieke uitzonderingen ingesteld.</p>;
  return <div className="max-h-48 divide-y divide-border overflow-y-auto rounded-md border border-border bg-card">
    {overrides.map(item => <div key={item.id} className="flex items-start gap-3 p-3">
