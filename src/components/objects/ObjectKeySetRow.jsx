...
export default function ObjectKeySetRow({ set, expanded, onToggle, onOpenHistory, onRequestDelete, disabled }) {
  return (
    <div>
      <div className="group grid cursor-pointer grid-cols-[200px_1fr_140px_40px] items-center px-4 py-3 transition-colors hover:bg-accent/30" onClick={onToggle}>
      ...
      <AnimatePresence>{expanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18, ease: "easeOut" }} className="overflow-hidden divide-y divide-border border-t border-border bg-muted/10">
        {set.keys.length ? set.keys.map(key => { const status = objectKeyStatus(key.status); return <div key={key.id} role="button" tabIndex={0} onClick={() => onOpenHistory(key)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") onOpenHistory(key); }} className="group grid cursor-pointer grid-cols-[200px_1fr_140px_40px] items-center px-4 py-3 transition-colors hover:bg-accent/30"><div className="flex min-w-0 items-center gap-2 pl-5"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-muted-foreground/40" /><span className="truncate text-sm font-medium text-foreground">{key.serial_number || "Geen serienummer"}</span></div><span className="truncate text-xs text-muted-foreground">{keyTypeLabel(key.key_type)} · {key.brand}</span><span className={`text-xs ${status.tone}`}>{status.label}</span><Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 transition-opacity group-hover:opacity-100" aria-label="Sleutel verwijderen" disabled={disabled} onClick={event => { event.stopPropagation(); onRequestDelete(key, set); }}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button></div>; }) : <div className="flex items-center gap-2 px-9 py-3 text-sm text-muted-foreground"><KeyRound className="h-4 w-4" /> Deze set bevat nog geen sleutels.</div>}
