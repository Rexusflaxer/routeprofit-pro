...
export default function ObjectSecurityPlanTab() {
  return <div className="flex min-h-[620px] flex-col bg-card/35 backdrop-blur-xl"><div className="border-b border-border/70 bg-card/25 px-5 py-4 backdrop-blur-xl"><h2 className="text-sm font-semibold text-foreground">Beveiligingsplan</h2><p className="mt-1 text-xs text-muted-foreground">De onderdelen van het beveiligingsplan voor dit object.</p></div><div className="space-y-3 p-5"><div><p className="text-sm font-medium text-foreground">Categorieën</p><p className="mt-1 text-xs text-muted-foreground">Het beveiligingsplan wordt per soort dienstverlening onderverdeeld.</p></div><div className="grid grid-cols-1 gap-2">{CATEGORIES.map(category => <article key={category.label} className="rounded-lg border border-border bg-card px-4 py-3"><p className="text-sm font-semibold text-foreground">{category.label}</p><p className="mt-1 text-xs text-muted-foreground">{category.description}</p></article>)}</div></div></div>;
}