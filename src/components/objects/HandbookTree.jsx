...
export default function HandbookTree({ categories, articles, selectedArticleId, onSelectCategory, onOpenArticle, onDelete, archived, deleting }) {
  ...
  const renderLevel = (parentId = null, depth = 0) => categories
    ...
    .map(category => {
      ...
      return <div key={category.id}>
        <button type="button" onClick={() => toggle(category)} className="flex w-full items-center gap-1.5 rounded-md py-2 pr-2 text-left text-sm font-medium hover:bg-muted/50" style={{ paddingLeft: `${8 + depth * 14}px` }}>
