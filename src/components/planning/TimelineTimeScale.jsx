...
export default function TimelineTimeScale({ startMinute, endMinute, boundaryMinutes = [], openBoundaryMinutes = [], positionForMinute = null }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-30" aria-hidden="true">
      ...
      {!positionForMinute && hours.map(minute => (
        <span key={`hour-${minute}`} className="absolute left-11 right-0 h-px bg-border/30" style={{ top: `${((minute - startMinute) / duration) * 100}%` }} />
      ))}
      {markers.map(minute => {
        const position = positionForMinute
          ? positionForMinute(minute)
          : ((minute - startMinute) / duration) * 100;
