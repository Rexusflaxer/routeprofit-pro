...
export default function ReportTemplates() {
  ...
  return (
    <PageTransition>
      <PageHeader title="Rapportagetemplates" subtitle="Beheer mobiele formulieren per taaktype" />
      <div className="bg-white border rounded-xl p-4 space-y-3">
        <Input placeholder="Naam" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/>
        <Input placeholder="Taaktype" value={form.task_type} onChange={e=>setForm({...form,task_type:e.target.value})}/>
        <Textarea placeholder="Velden JSON" rows={7} value={form.fields} onChange={e=>setForm({...form,fields:e.target.value})}/>
        <Button onClick={save}>Template opslaan</Button>
      </div>
      <div className="grid gap-3">
        {templates.map(t=>(
          <div key={t.id} className="bg-white border rounded-xl p-4">
            <p className="font-semibold">{t.name}</p>
            <p className="text-sm text-slate-500">{t.task_type}</p>
          </div>
        ))}
      </div>
    </PageTransition>
