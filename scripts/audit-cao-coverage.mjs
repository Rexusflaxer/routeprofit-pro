...
const requiredExplicitLevels = ['automatic_or_calculation', 'validation_or_policy', 'workflow_or_documentation'];
const summaries = [
  ...requiredExplicitLevels,
  'reference_or_policy',
  'reference'
].map(level => summarizeLevel(rules, repoText, runtimeIds, level));

const requiredExplicitMissing = rules.filter(rule =>
  requiredExplicitLevels.includes(rule.automation_level) &&
  !repoText.includes(rule.rule_id)
);
