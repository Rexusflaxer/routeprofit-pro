...
De melding **customer_id is verplicht** bij de collectiefkaart is een tweede variant: oudere kaartacties bestaan al, maar kennen alleen klantobjecten. Herstel uitsluitend na **Onbekende actie** is daarom niet genoeg. Een collectief bewaart zijn kaart via `collective_id`, onafhankelijk van de optionele beheerderskoppeling. Ook bij een gekoppelde beherende klant mogen geen `customer_id` of `object_id` aan deze kaartaanvraag worden toegevoegd. De opgeslagen `manager_customer_id` en eventuele oude `customer_id` blijven behouden.
...
Neem de correctie in `src/pages/Collectief.jsx` en `src/components/customers/customerDossierUtils.js` over, inclusief de regressietests `CollectiveDossier.test.jsx`, `CustomerPlatformRuntime.test.js`, `CollectiveMapRuntime.test.js` en `collectiveMapBackend.test.js`. Zorg dat de reeds toegevoegde collectiefentiteiten en de handler in `customerPlatformApi/collectiveDossier.ts` met de registratie in `entry.ts` daadwerkelijk zijn gesynchroniseerd en gepubliceerd.

Routeer in vastgezette previews alle vijf collectiefreads en zes collectiefmutaties uit de serverregistratie **direct** naar de actuele functions-client. Doe dit ook voor `get_object_map_configuration`, `list_object_building_candidates`, `list_object_parcel_candidates` en `update_object_map_configuration` met een expliciete, uitsluitend collectiefgebonden scope. Gebruik dezelfde authenticatieconfiguratie, volledige payload, `expected_version` en `idempotency_key`. Selecteer de client vóór het verzoek, niet na een mislukte mutatie. Ongepinde omgevingen, gewone klantobjecten en commerciële acties behouden hun bestaande client. De bestaande objectmodule-herstelroute bij exact HTTP 400 **Onbekende actie** blijft ongewijzigd.

Herhaal geen validatiefout, bevoegdheidsfout, versieconflict, timeout of serverstoring op een andere backend. Gebruik geen directe entiteit-CRUD als alternatief. Als de actuele backend de actie of de collectiefkaartscope nog niet kent, toon synchronisatie-/publicatie-instructies en behoud status en foutreferentie. Deze correctie verandert geen entiteiten, rechten, backendcontracten of iOS-code.
...
3. Test de oude previewversie: collectievenlijst, kaartconfiguratie, BAG-/perceelkandidaten en opslaan gebruiken direct de actuele backend. De oude object-only kaartactie wordt niet aangeroepen; een collectiefmutatie wordt niet op een andere backend herhaald. Versie en idempotencysleutel blijven intact.
...
6. Controleer de collectiefkaart met een beherende klant, met een oude klantkoppeling en zonder beheerder. Lezen en opslaan blijven op `collective_id` werken; de bestaande klantkoppeling en klantobjectkaarten veranderen niet.
